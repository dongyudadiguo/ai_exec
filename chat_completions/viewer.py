...
def repair_unclosed_tool_calls(path=None):
    """Close any assistant tool_calls that lack a matching role:"tool" message.

    A run can be interrupted (manual stop during a long tool call, crash, ...)
    after ae.py appended the assistant message with tool_calls but before it
    wrote the tool outputs, leaving an unanswered tool call which the Chat
    Completions API rejects. Append a placeholder tool message for each
    ...
    calls = [c for m in messages if m.get("tool_calls")
             for c in m["tool_calls"] if c.get("id") not in answered]
    if not calls:
        return 0
    for call in calls:
        messages.append({
