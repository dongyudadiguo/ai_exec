...
def file_part_from_storage(storage):
    ...
    try:
        ...
        encoded = base64.b64encode(raw).decode("ascii")
        return {"type": "file", "file": {"filename": filename, "file_data": f"data:{mime};base64,{encoded}"}}
...
def repair_unclosed_tool_calls(path=None):
    ...
    calls = [
        c for m in messages if m.get("tool_calls")
...
def chat_transcript(body):
    ...
    for message in body.get("messages", []):
        if not isinstance(message, dict) or not message.get("role"):
            continue
        item = {"role": message["role"], "content": _chat_content_for_display(message.get("content"))}
...
def load_cached(path=None):
    ...
    if prev is not None and prev.get("items") is not None and prev.get("messages") is not None:
        ...
        if len(items) >= len(prev_items) and items[: len(prev_items)] == prev_items:
            ...
            messages = prev_msgs if not delta else list(prev_msgs) + chat_transcript({"messages": delta})
    if messages is None:
