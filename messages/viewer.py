...
def file_part(part):
    """Create one native Anthropic Messages user-content block."""
...
def _user_message_text(message):
    """Extract editable text from a native Anthropic human user message."""
    content = message.get("content", "")
    if isinstance(content, str):
        return content
...
def append_user_message(text, files, path=None):
    """Append a native Anthropic Messages human user message."""
    data = read_input(path)
    body = data.setdefault("json", {})
    messages = body.setdefault("messages", [])
    blocks = []
...
def repair_unclosed_tool_calls(path=None):
    """Close any tool_use blocks that lack a matching tool_result block.

    A run can be interrupted (manual stop during a long tool call, crash, ...)
    after ae.py appended the assistant tool_use blocks but before it wrote the
    tool results, leaving an unanswered tool_use which the Messages API rejects.
    Append a user message with placeholder tool_result blocks for each dangling
    call so the conversation can resume without hand-editing the JSON.
...
def anthropic_transcript(body):
    """Read only a native Anthropic Messages transcript."""
    result = []
    ...
    for message in body.get("messages", []):
        ...
        if role == "assistant":
            ...
            item = {
                "role": "assistant",
                "content": _anthropic_content_for_display(content),
            }
            if calls:
...
def display_message(m):
    ...
    if m.get("role") == "tool":
        ...
        d["tool_failed"] = bool(m.get("tool_failed") or re.search(
    ...
    if prev is not None and prev.get("items") is not None and prev.get("messages") is not None:
        ...
        if len(items) >= len(prev_items) and items[:len(prev_items)] == prev_items:
            ...
            if not delta:
                ...
                tail_msgs = anthropic_transcript({"messages": delta})
...
<title>Anthropic Messages · input.json 查看器</title>
<script>
