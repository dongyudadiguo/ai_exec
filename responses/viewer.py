...
def runner_idle(chat_id=None):
    ...
    transcript itself: the last message is a completed assistant text message
    (no unanswered function_call).  So viewer.py and ae.py coordinate purely
    through the shared input.json — no extra files or signals needed.
    """
...
def runner_phase(messages, is_running):
    ...
    - after a round with no tool calls, ae.py stays alive and waits for the
      ...
      text message, which we report as idle_wait ("等待消息").
    """
    if not is_running:
        return {
            "phase": "idle",
...
def repair_unclosed_tool_calls(path=None):
    """Close any function_call items that lack a matching function_call_output.

    A run can be interrupted (manual stop during a long tool call, crash, ...)
    after ae.py wrote the assistant's function_call but before it wrote the
    tool output, leaving the transcript ending in an unanswered function_call
    which the Responses API rejects. Append a placeholder output for each
    dangling call so the conversation can resume without hand-editing the JSON.
    Returns the number of calls closed (0 if nothing to do).
    """
    target = INPUT_FILE if path is None else Path(path)
    try:
    ...
    body = data.setdefault("json", {})
    items = body.setdefault("input", [])
    answered = {it.get("call_id") for it in items if it.get("type") == "function_call_output"}
    if not any(it.get("type") == "function_call" and it.get("call_id") not in answered for it in items):
        return 0
    new_items = []
    repaired = 0
    for it in items:
        ...
        if it.get("type") == "function_call" and it.get("call_id") not in answered:
            new_items.append({
                ...
                "output": "（工具调用被中断，未返回结果，继续运行时自动补全）",
            })
...
def response_transcript(body, seed_calls=None):
    """Convert Responses API input/output items to the viewer's chat-like shape.

    Returns (messages, tail_calls). tail_calls lists the function calls that
    form a trailing assistant tool_calls group (empty otherwise). load_cached
    keeps tail_calls so an append-only rewrite can resume the conversion
    incrementally instead of re-parsing the whole transcript.
    """
    result = []
    ...
    pending_calls = list(seed_calls) if seed_calls else []
    tail_calls = []

    def flush_calls(record_tail=False):
        nonlocal pending_calls, tail_calls
        if pending_calls:
            result.append({"role": "assistant", "tool_calls": pending_calls})
            if record_tail:
                tail_calls = list(pending_calls)
            pending_calls = []
    ...
    for item in body.get("input", []):
        ...
        if role:
            ...
            if isinstance(content, list):
                ...
                for part in content:
                    part_type = part.get("type")
                    if part_type in ("input_text", "output_text", "refusal"):
                        ...
                        parts.append({"type": "text", "text": f"[附件：{part.get('filename', 'file')}]"})
                content = parts[0]["text"] if len(parts) == 1 and parts[0].get("type") == "text" else parts
            result.append({"role": role, "content": content})

    flush_calls(record_tail=True)
    return result, tail_calls
...
def load_cached(path=None):
    ...
    # Falls back to a full conversion when the rewrite is not purely append-only
    # (compaction, repair, reorder, ...) or the cache is inconsistent.
    messages = None
    tail_calls = []
    with _state_cache_lock:
    ...
    if prev is not None and prev.get("items") is not None and prev.get("messages") is not None:
        ...
        prev_tail = prev.get("tail_calls") or []
        if len(items) >= len(prev_items) and items[:len(prev_items)] == prev_items:
            delta = items[len(prev_items):]
            if not delta:
                messages, tail_calls = prev_msgs, prev_tail
            elif not prev_tail:
                tail_msgs, tail_calls = response_transcript({"input": delta})
                messages = list(prev_msgs) + tail_msgs
            elif prev_msgs and prev_msgs[-1].get("role") == "assistant" and prev_msgs[-1].get("tool_calls") is not None:
                # Undo the previous tail flush (trailing assistant tool_calls) first.
                tail_msgs, tail_calls = response_transcript({"input": delta}, seed_calls=prev_tail)
                messages = list(prev_msgs[:-1]) + tail_msgs
    if messages is None:
        messages, tail_calls = response_transcript(body)
...
<title>Responses · input.json 查看器</title>
<script>
        ...
        try:
            ...
            if path == "/api/send":
                ...
                with _send_lock:
                    ...
                    # never submits an unanswered function_call to the API.
