# -*- coding: utf-8 -*-
"""input.json viewer — Flask + psutil (Termux: local stub), static assets out of this file."""
...
HOST = os.environ.get("AE_VIEWER_HOST", "127.0.0.1")
...
def pop_last_user_message(path=None):
    ...
    items = data.setdefault("json", {}).setdefault("input", [])
    if not items:
...
def response_transcript(body, seed_calls=None):
    ...
    if body.get("instructions"):
        result.append({"role": "system", "content": body["instructions"]})
    ...
    for item in body.get("input", []):
        ...
        if kind == "function_call":
            pending_calls.append({
                "id": item.get("call_id"),
                "function": {"name": item.get("name", "python"), "arguments": item.get("arguments", "")},
            })
            tail_calls = []
            continue
        flush_calls()
        ...
        if kind == "function_call_output":
            result.append({"role": "tool", "tool_call_id": item.get("call_id"), "content": item.get("output", "")})
            continue
        role = item.get("role")
        if role:
            content = item.get("content", "")
...
def display_message(m):
    ...
    if m.get("role") == "tool":
        ...
        d["tool_failed"] = bool(_TOOL_FAIL_RE.search(c))
    if m.get("tool_calls"):
        d["tool_calls"] = [
...
if __name__ == "__main__":
    ...
    if sys.stdout:
        ...
        print(f"Termux: termux-open-url {url}")
